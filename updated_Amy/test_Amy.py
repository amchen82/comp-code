...
async def main():
   if hub.imu.ready():
    ...
    await robot.turn(-90)
    await multitask(RAM.run_until_stalled(300,duty_limit=25),LAM.run_until_stalled(300, duty_limit=30)) #arm goes down (LAM+RAM)
    # await multitask(RAM.run_time(690,1060),LAM.run_time(690, 1069))
   
    print("flipper dn, fork  dn")
  ...
  # inside cave 
    ...
    # await LAM.run_time(690,400) #arm goes down (LAM+RAM)
    ...
    # await multitask(RAM.run_time(-690,1110),LAM.run_time(-690,730))  # arm go up (LAM)
    await LAM.run_time(-690,550)# arm go up (LAM)
    wait(500)
    await RAM.run_time(-690,1000)
